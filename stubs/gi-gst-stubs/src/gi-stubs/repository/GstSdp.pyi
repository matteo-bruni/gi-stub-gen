...
class MIKEYMessage(GObject.GBoxed, metaclass=GObject.GType):
    ...
    def add_payload(self, payload: MIKEYPayload) -> bool:
        ...
        """
    def add_pke(self, C: MIKEYCacheType, data_len: int, data: bytes) -> bool:
        """
        Add a new PKE payload to `msg` with the given parameters.
        """
    def add_rand(self, len: int, rand: bytes) -> bool:
        """
...
class MIKEYPayload(GObject.GBoxed, metaclass=GObject.GType):
    ...
    def kemac_set(self, enc_alg: MIKEYEncAlg, mac_alg: MIKEYMacAlg) -> bool:
        ...
        payload.
        """
    def key_data_set_interval(self, vf_len: int, vf_data: bytes, vt_len: int, vt_data: bytes) -> bool:
        """
        Set the key validity period in the GST_MIKEY_PT_KEY_DATA `payload`.
        """
    def key_data_set_key(self, key_type: MIKEYKeyDataType, key_len: int, key_data: bytes) -> bool:
        """
            Set `key_len` bytes of `key_data` of type `key_type` as the key for the
        GST_MIKEY_PT_KEY_DATA `payload`.
        """
    def key_data_set_salt(self, salt_len: int, salt_data: bytes | None = None) -> bool:
        """
            Set the salt key data. If `salt_len` is 0 and `salt_data` is None, the
        salt data will be removed.
        """
    def key_data_set_spi(self, spi_len: int, spi_data: bytes) -> bool:
        """
        Set the SPI/MKI validity in the GST_MIKEY_PT_KEY_DATA `payload`.
        """
    def pke_set(self, C: MIKEYCacheType, data_len: int, data: bytes) -> bool:
        """
            Set the PKE values in `payload`. `payload` must be of type
        GST_MIKEY_PT_PKE.
        """
    def rand_set(self, len: int, rand: bytes) -> bool:
        """
        Set the random values in a GST_MIKEY_PT_RAND `payload`.
        """
    def sp_add_param(self, type: int, len: int, val: bytes) -> bool:
        """
...
class SDPMessage(GObject.GBoxed, metaclass=GObject.GType):
    ...
    def new_from_text(text: str) -> tuple[SDPResult, SDPMessage]:
        ...
        """
    @staticmethod
    def parse_buffer(data: bytes, size: int, msg: SDPMessage) -> SDPResult:
        """
            Parse the contents of `size` bytes pointed to by `data` and store the result in
        `msg`.
        """
