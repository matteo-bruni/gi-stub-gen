...
def normalize_optional_types(signature: str) -> str:
    """Normalize Optional[X] to X | None for consistent comparison.

    This treats Optional[X] and X | None as equivalent.
    """
    # Pattern to match Optional[...] including nested brackets
    # We need to handle nested brackets like Optional[Dict[str, int]]
    result = signature

    # Simple approach: repeatedly replace Optional[X] with X | None
    # Handle nested brackets by counting
    while "Optional[" in result:
        start = result.find("Optional[")
        if start == -1:
            break

        # Find the matching closing bracket
        bracket_count = 0
        end = start + len("Optional[")
        for i, char in enumerate(result[start + len("Optional[") :], start=start + len("Optional[")):
            if char == "[":
                bracket_count += 1
            elif char == "]":
                if bracket_count == 0:
        ...
        inner = result[start + len("Optional[") : end]
        # Replace with X | None
...
def generate_markdown_report(
    ...
    lines = [
        ...
        "",
        f"**Generated by:** gi-stub-diff v{__version__}",
        f"**Date:** {current_date}",
        "",
        "## Description",
        "",
    ...
    for class_name in sorted(set(stub1.class_members.keys()) & set(stub2.class_members.keys())):
        ...
        for member_name in sorted(set(members1.keys()) - set(members2.keys())):
            ...
            only_in_1.append((elem.kind, f"{class_name}.{member_name}", elem.signature))
    ...
    if only_in_1:
        ...
        for kind, name, sig in only_in_1:
            lines.append(f"| {kind} | `{name}` | {escape_markdown(sig, indent_style)} |")
    else:
    ...
    for class_name in sorted(set(stub1.class_members.keys()) & set(stub2.class_members.keys())):
        ...
        for member_name in sorted(set(members2.keys()) - set(members1.keys())):
            ...
            only_in_2.append((elem.kind, f"{class_name}.{member_name}", elem.signature))
    ...
    if only_in_2:
        ...
        for kind, name, sig in only_in_2:
            lines.append(f"| {kind} | `{name}` | {escape_markdown(sig, indent_style)} |")
    else:
    ...
    for name in sorted(common_names):
        ...
        if elem1 and elem2:
            # Compare signatures, optionally normalizing Optional types
            ...
            # Check if kind or signature differs
    ...
    lines.extend(
        [
            ...
            "# Summary",
            "",
            f"- Elements only in {display_name1}: **{len(only_in_1)}**",
            f"- Elements only in {display_name2}: **{len(only_in_2)}**",
            f"- Elements with differences: **{len(differences)}**",
            "",
        ]
    )
